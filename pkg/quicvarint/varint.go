...
// Write i in the QUIC varint format to writer.
func Write(w io.Writer, i uint64) (n int, err error) {
	if i <= maxVarInt1 {
		return w.Write([]byte{uint8(i)})
	}
	if i <= maxVarInt2 {
		return w.Write([]byte{uint8(i>>8) | 0x40, uint8(i)})
	}
	if i <= maxVarInt4 {
		return w.Write([]byte{uint8(i>>24) | 0x80, uint8(i >> 16), uint8(i >> 8), uint8(i)})
	}
	if i <= maxVarInt8 {
		return w.Write([]byte{
			uint8(i>>56) | 0xc0, uint8(i >> 48), uint8(i >> 40), uint8(i >> 32),
	...
	return 0, fmt.Errorf("%#x doesn't fit into 62 bits", i)
}
